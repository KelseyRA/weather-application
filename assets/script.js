	...
	fetch(queryURL)
		...
		.then(function (data) {
			...
			document.querySelector(".temp").innerHTML =
				"<strong>Temp: </strong>" + Math.floor(temp) + " °F";
			document.querySelector(".description").innerHTML =
				"<strong>Cloud Coverage: </strong>" + description;
	...
	fetch(fiveDayQ)
		...
		.then(function (data) {
			...
			for (var i = 0; i < data.list.length; i = i + 8) {
				...
				tempOne.innerHTML =
					"<strong>Temp: </strong>" +
					Math.floor(data.list[i].main.temp) +
					" °F";
				humidityOne.innerHTML =
					"<strong>Humidity: </strong>" + data.list[i].main.humidity + " %";
				windOne.innerHTML =
					"<strong>Wind: </strong>" + data.list[i].wind.speed + " MPH";
